...
export const useQuickStats = () => {
  return useQuery({
    ...
    // Transform the data in the caching layer before it hits your UI
    select: (issues: any[]) => {
      const total = issues.length;
      
      const fixed = issues.filter(
        (i: any) => i.status === 'resolved' || i.status === 'fixed'
      ).length;
        ...
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 items-center lg:items-start">
          ...
          <div className="w-full lg:w-3/5 grid grid-cols-1 justify-center sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 relative">
            {stats.map((stat, index) => (
              <div key={index} className={`relative flex flex-col justify-between p-6 min-h-40 w-full bg-primary/10 border border-white/5 rounded-2xl animate-pulse ${stat.className}`}>
                <div className="absolute top-0 right-0 w-1.5 h-full bg-secondary rounded-r-2xl opacity-40" />
                <div className="flex justify-between items-start">
                  <div className="h-2.5 w-20 bg-primary/20 rounded" />
                  <div className="w-4 h-4 bg-primary/20 rounded-full" />
                </div>
                <div className="mt-6">
                  <div className="h-8 w-16 bg-primary/20 rounded-lg mb-2" />
