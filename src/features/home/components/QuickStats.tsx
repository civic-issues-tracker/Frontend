      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 items-center lg:items-start">
        ...
        <div className="w-full lg:w-2/5 text-center lg:text-left">
          ...
          <h2 className="text-3xl md:text-5xl font-header font-black text-primary leading-tight mb-4">
            Our Civic <br className="hidden lg:block" /> Impact
          </h2>
          
          <p className="text-primary/60 text-[15px] leading-relaxed mb-6 max-w-xs mx-auto lg:mx-0 font-body">
            A real-time overview of the Civic Issues Tracker platform, 
            showcasing our commitment to efficient municipal management.
          </p>
        ...
        <div className="w-full lg:w-3/5 grid grid-cols-1  justify-center sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 relative">
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className={`relative flex flex-col justify-between p-6 min-h-40 w-full 
                bg-primary rounded-2xl shadow-xl transition-all duration-500 
                hover:-translate-y-2 ${stat.className}`}
            >
              <div className="absolute top-0 right-0 w-1.5 h-full bg-secondary rounded-r-2xl opacity-80" />

              <div className="flex justify-between items-start">
                <p className="text-[10px] font-black tracking-widest text-secondary/50 leading-tight uppercase">
                  {stat.label}
                </p>
                {stat.icon}
              </div>
              ...
              <div className="mt-6">
                <h3 className="text-4xl font-black font-header text-secondary tracking-tighter mb-1">
                  {stat.value}
                </h3>
